...
| **Statistical rigor**: appropriate uncertainty + comparisons? | 🟡 | *"Numbers in gray color are values with p-value larger than 0.05."* `Table 2 caption, p.3`, per-cell significance is flagged for the correlation task, but no confidence intervals or multiple-comparison correction are reported alongside this |
