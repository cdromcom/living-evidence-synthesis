...
| **Statistical rigor**: appropriate uncertainty + comparisons? | 🟡 | *"all results reported are averaged over 20 independent runs"* `§5, p.9`, repeated-run averaging and paired t-tests are used, but no multiple-comparison correction across the seven systems × three datasets × multiple metrics is reported |
