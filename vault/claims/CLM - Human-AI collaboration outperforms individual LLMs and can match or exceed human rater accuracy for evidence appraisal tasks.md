---
NodeFormality: draft
TruthValue: 0.5
aliases:
tags:
  - 5c/clarity
  - 5c/credibility
created: 2026-04-17
updated: 2026-04-29
nodeID: 019ddb4e-6b95-73a2-8f55-e475c8be6b8e
---
### Key Papers Making This Claim

> [!info] Papers
> [[@woelfleBenchmarkingHumanAICollaboration2024]]

### Snippet: Quote(s) & Screenshots

> [!info] Quotes
> "Current LLMs alone appraised evidence substantially worse than humans. Pairing a first human rater with an LLM as human−AI collaboration may reduce workload for the second human rater for the assessment of reporting (PRISMA) and methodological rigor (AMSTAR) while maintaining very high accuracy but not for more complex tasks such as assessing pragmatism of clinical trials (PRECIS-2)." (Woelfle et al., 2024, p. 10, Conclusion)
>
> The conclusion paragraph spans the bottom of the left column and the top of the right column on p. 10:
>
> ![[woelfleBenchmarkingHumanAICollaboration2024-quote-conclusion-p10a-10.png]]
> ![[woelfleBenchmarkingHumanAICollaboration2024-quote-conclusion-p10b-10.png]]
>
> Supporting figure (Fig 2, p. 4): confusion matrices showing human-AI collaboration's accuracy gain for PRISMA/AMSTAR but flat performance for PRECIS-2.
>
> ![[woelfleBenchmarkingHumanAICollaboration2024-fig2fig3-p4-04.png]]

### Supporting Evidence

> [!info] EVDs from the cited papers (auto-derived; not all may directly support the claim — verify before citing)

- [[EVD - Combined LLMs with consistency approach reached 75-88 percent accuracy for PRISMA while deferring 4-74 percent of ratings - @woelfleBenchmarkingHumanAICollaboration2024]]

- [[EVD - GPT-4 cost approximately 100x more than Mixtral-8x22B per 100 papers in evidence appraisal - @woelfleBenchmarkingHumanAICollaboration2024]]

- [[EVD - Human inter-rater reliability dropped from 91 percent on PRISMA to 57 percent on PRECIS-2 - @woelfleBenchmarkingHumanAICollaboration2024]]

- [[EVD - Human-AI collaboration achieved up to 96 percent accuracy for PRISMA and 95 percent for AMSTAR surpassing individual human raters - @woelfleBenchmarkingHumanAICollaboration2024]]

- [[EVD - Individual LLM accuracy ranged 63-70 percent for PRISMA and 53-74 percent for AMSTAR versus 89 percent for humans - @woelfleBenchmarkingHumanAICollaboration2024]]

- [[EVD - Median quote similarity with original full text was 99 percent across LLM extracted quotes - @woelfleBenchmarkingHumanAICollaboration2024]]
